 ...
 *
 * @return The copied string on success and NULL on failure. The error is set.
 */
 ...
 * that replaces any special characters such as \\n and \\t with their code
 * representation.
 *
 * @brief src remains untouched even if the function fails.
 *
 ...
 *
 * @brief idx is wrapped to the end if it is negative.
 *
 ...
 * returning zero. base can be any number between 2 and 36, where above 10
 * letters start to be used. If the base is 0 the function accepts prefixes
 * such as 0x, 0b and 0o to change the base.
 *
 ...
 *
 * @brief If an invalid literal is found, the function fails rather than
 * returning zero. Valid literals follow Nest's real syntax, because of this 3.
 * or .5 are considered invalid and must be written as 3.0 and 0.5.
 *
 * @param str: the string to parse
 *
 ...
 *
 * @return 0 if the two strings are equal, a value < 0 if str2 is greater than
 * str1 and a value > 0 if str1 is greater than str2.
 */
 ...
 * @param l2: the lenght of s2
 *
 * @return The pointer to the start of s1 or NULL if the string could not be
 * found. No error is set.
 */
