...
/**
 * Struct representing the current state of the Nest interpreter.
 *
 ...
 *
 * @param file_name: the name of the file to run, it must exist
 *
 * @return A map containint the variables of the module or NULL on failure. The
 * error is set.
 */
...
/**
 * Yield a coroutine. If `stack` is `NULL` the function will only set
 * `out_stack_size` and return without modifying the state of the interpreter.
 * This is to allow the caller to allocate enough memory to hold the stack.
 *
...
/* Get the current intepreter state. */
