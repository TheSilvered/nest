...
/*
  ...
  NOTE: using together '+' and ' ' or using together '-' and '0' results in an
        invalid sequence
  ...
  - for d, i, u, x, X, o it specifies the minimum number of digits that the
    number must contain
  ...
  - g, G: prints a float removing the trailing zeroes and using the standard
          form when using more than the specified significant digits
 ...
 *
 * @return The function returns the number of characters written. On failure
 * a negative value is returned depending on the type of the error: -1 signals
 * a failure of vsprintf, -2 that the output file is closed, -3 an error in the
 * format string and -4 a memory allocation error. No error is set.
 */
