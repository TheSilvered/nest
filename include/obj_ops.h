...
/**
 * @brief Alias for _Nst_obj_cast that casts ob to Nst_Obj * and type to
 * Nst_TypeObj *.
 */
 ...
 *
 * @brief It is recommended to use Nst_obj_cast(seq_obj, Nst_type()->Str), this
 * is used internally.
 *
 ...
 *
 * @brief It is recommended to use Nst_obj_cast(map_obj, Nst_type()->Str), this
 * is used internally.
 *
 ...
 *
 * @brief Casting an object to Bool is guaranteed to not fail. All objects can
 * be casted to Str but the function can still fail if a memory allocation
 * fails.
 *
 * @param ob: the object to be casted
 ...
 * function failes if the specified library is not found.
 */
