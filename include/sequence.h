...
/*
  ...
  - I: a 64 bit integer
  - i: a 32 bit integer
  - f, F: a float or a double
  - b: a boolean (promoted to an int)
  - B: a byte (promoted to an int)
  - o: an already existing object to take one reference from
  - O: an already existing object to add one reference to
  - n: null, the vararg can be any pointer as its value is ignored but NULL is
       preferred
*/
...
/**
 * @brief Alias for _Nst_seq_set that casts seq to Nst_SeqObj * and val to
 * Nst_Obj *.
 */
...
/**
 * @brief Alias of _Nst_vector_append that casts vect to Nst_SeqObj * and val
 * to Nst_Obj *.
 */
#define Nst_vector_append(vect, val) _Nst_vector_append(SEQ(vect), OBJ(val))
/**
 * @brief Alias of Nst_vector_remove that casts vect to Nst_SeqObj * and val to
 * Nst_Obj *.
 */
 ...
 *
 * @brief The objects inside the array must be set manually, each entry in the
 * array takes one reference of the object inserted.
 *
 ...
 *
 * @brief The objects inside the vector must be set manually, each entry in the
 * vector takes one reference of the object inserted.
 *
 ...
 *
 * @brief The number of varargs passed to the function must match the number
 * of types in the fmt argument. For more information about object types in
 * Nst_array_create_c see sequence.h.
 *
 ...
 * @brief The number of varargs passed to the function must match the number
 * of types in the fmt argument. For more information about object types in
 * Nst_vector_create_c see sequence.h.
 *
 * @param fmt: the types of the values passed to the function
 * @param ...: the values passed to the function used to create the objects
 *
 ...
 *
 * @brief The sequence to change must already contain valid values. idx can be
 * negative in which case it is subtracted from the length of the sequence to
 * get the new index.
 *
 ...
 *
 * @return A new reference to the object at index on success and NULL on
 ...
 *
 * @return true on success and false on failure. The error is set. The function
 * never fails if the vector is untouched or is shrinked.
 */
 ...
 *
 * @return true if the object was removed and false if there was no object that
